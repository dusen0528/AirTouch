...
## v0.3.6 — 손동작 연속성과 화면 갱신

- 짧은 클릭 확인과 클릭 직후 작은 흔들림 흡수, 두 손가락 스크롤 직접 재개, 일시적 스크롤 자세 불확실성에서 출력 동결 후 재개를 적용했다.
- 카메라 결과는 대기 중 최신 것 하나로 합치며, 진단 화면 갱신은 입력 처리와 분리했다.
- 실제 v0.3.5 스크롤 기록 재생에서 재개 확인 0/9 → 9/9. 자동 테스트 73개 통과·이전 원본 전용 1개 건너뜀.
- 설치본 합성 재생: 클릭 1, 드래그 완료 1, 스크롤 약 354pt, 버튼 해제. 이는 실제 손/OS 입력 검증이 아니다.
- 내장 카메라 측정에서 연속 실행은 확인했으나 전체 지연 감소는 확인되지 않았다. [세부 기록](verification/v036-gesture-usability.md).
- v0.3.6 build 9 하나만 설치·등록·실행 중임을 확인했다. 기존 서명 요구식과 카메라·손쉬운 사용 승인은 유지됐다.


## v0.4.0 — 개인 보정·끌기 잠금·커서 상태

- 사용자가 선택한 세 가지 사용성 개선과 UI와 분리된 입력 판정을 적용했다.
- 720p 요청이 실제로는 1080p로 바뀌던 카메라 형식 문제를 실측해 수정했다. 720p/480p/720p 실제 버퍼와 시간을 비교했다.
- 자동 테스트 117개 통과·원본 미제공 1개 건너뜀. 보정 화면 렌더와 설치본 합성 시연을 확인했다. 실제 손 조작의 편안함과 대상 앱 입력 성공은 별도 확인 대상이다.
- v0.4.0 build 10 단일 설치·등록·일반 실행, 기존 개발 서명 요구식과 카메라·손쉬운 사용 승인을 확인했다. 휴지통에 남았던 옛 앱의 실행 등록을 해제했다.
- [구현·측정·검증 기록](verification/v040-personal-control.md), [설치 상태](verification/v040-install-state.json).
