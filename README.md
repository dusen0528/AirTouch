...
**현재 버전: v0.4.5 · macOS 14 이상 · Swift / SwiftUI / AppKit**
...
```mermaid
flowchart TD
    A[소스 다운로드 · 설치] --> B[사용 준비]
    B --> C[카메라 · 손쉬운 사용 허용]
    C --> D[긴급 중지 단축키 확인]
    D --> E[내 손에 맞추기]
    E --> E1[정지 → 좌우 → 상하 → 세 번 집기]
    E1 --> E2[보정 적용]
    E2 --> F[손동작 연습]
    F --> G[메뉴 막대 → 전체 제어 시작]
    G --> H[2초 준비 · 제어 창 숨김]
    H --> I[이동 · 클릭 · 드래그 · 스크롤]
    I --> J{다음 동작}
    J -->|손 내리기 · 손바닥 펼치기| K[쉬기 · 눌린 입력 해제]
    K -->|검지를 펴기| I
    J -->|마우스 · 트랙패드 사용| L[손 제어 잠시 양보]
    L -->|사용을 멈추고 1.5초 후 검지 자세| I
    J -->|긴급 단축키 · 메뉴에서 중지| M[카메라와 제어 중지]
    M --> N[메뉴 막대에서 대기]
    N -->|다시 시작| G
```
...
v0.4.5 검증 기록은 **자동 테스트 141개 통과·사용자 원본이 필요한 2개 건너뜀**입니다. 합성 입력 테스트, 실제 카메라 캡처, 대상 앱의 실제 입력 성공과 사용감은 서로 다른 검증 범위입니다. 세부 조건은 [v0.4.5 검증 기록](docs/verification/v045-pinch-continuity.md)과 [v0.4.4 카메라 검증](docs/verification/v044-camera-and-calibration.md)을 참고하세요.
